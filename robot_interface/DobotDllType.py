"""
Dobot DLL Type Definitions and API Wrapper

This module provides Python bindings for the Dobot Magician robotic arm DLL API.
It defines all data structures, enumerations, and function wrappers needed to 
communicate with the Dobot robot through its C library.

Key components:
- Data structures (ctypes.Structure) for robot commands and parameters
- Enumerations for robot modes and states
- DLL loading and function mapping
- High-level Python wrapper functions for robot control

The module uses ctypes to interface with the native Dobot DLL and provides
Pythonic access to all robot functionality including motion control, I/O,
sensors, and configuration.

Author: Dobot Robotics (Modified)
Compatible with: Dobot Magician, Magician Lite
"""

from ctypes import Structure, c_int, c_bool, c_float, c_byte, c_uint8, c_uint16, c_uint32, c_uint64, c_char_p, c_int32, c_ubyte, byref, create_string_buffer, addressof, c_ushort, cdll, CDLL, RTLD_GLOBAL
import math
import time
import platform
...
def enum(**enums):
    ...
    Usage: MyEnum = enum(Value1=0, Value2=1, Value3=2)
    Access: MyEnum.Value1 returns 0
    """
...
ParamsMode = enum(JOG=0,              # Jogging/manual control mode
                  other=1)            # Other parameter modes

# ==================== Device Information Structures ====================

class DevInfo(Structure):
    """Device information including ID, type, firmware, and runtime."""
    _pack_ = 1
    _fields_ = [
        ("devId", c_int),
        ("type", c_int),
...
class ConnectInfo(Structure):
    """Connection information for master and slave devices (multi-robot setups)."""
    _pack_ = 1
    _fields_ = [
...
class UpgradeFWReadyCmd(Structure):
    """Firmware upgrade command structure with size and MD5 checksum."""
    _pack_ = 1
    _fields_ = [
        ("fwSize", c_uint32),
        ("md5", c_char_p)
    ]
"""Unique device identifier (3-part ID)."""
    _pack_ = 1
    _fields_ = [
        ("deviceID1", c_uint32),
        ("deviceID2", c_uint32),
...
# Device role identifiers for multi-robot configurations
masterId = 0  # Master device ID
slaveId = 0   # Slave device ID
...
class EndTypeParams(Structure):
    """End effector offset parameters (tool center point calibration)."""
    _pack_ = 1
    _fields_ = [
        ("xBias", c_float),  # X-axis offset in mm
        ("yBias", c_float),  # Y-axis offset in mm
        ("zBias", c_float)   # Z-axis offset in mm
    ]

class Pose(Structure):
    """
    Complete robot pose including Cartesian coordinates and joint angles.
    Represents both forward kinematics (joints -> position) and 
    inverse kinematics (position -> joints) solutions.
    """oat)
    ]
...
class Pose(Structure):
    ...
    _fields_ = [
        ("x", c_float),
    """Basic kinematic parameters for motion."""
    _pack_ = 1
    _fields_ = [
        ("velocity", c_float),      # Linear/angular velocity
        ("acceleration", c_float)   # Linear/angular acceleration
    ]

class AlarmsState(Structure):
    """Robot alarm/error state bitfield."""
    _pack_ = 1
    _fields_ = [
        ("alarmsState", c_int32)  # Bit flags for various alarm conditions
    ]

# ==================== Homing and Calibration ====================

class HOMEParams(Structure):
    """Home position coordinates (where robot returns during homing)."""),
...
class AlarmsState(Structure):
    ...
    _fields_ = [
        ("alarmsState", c_int32)
    """Command to execute homing sequence."""
    _pack_ = 1
    _fields_ = [
        ("temp", c_float)  # Temporary/reserved parameter
    ]

class AutoLevelingCmd(Structure):
    """Auto-leveling command for ensuring level work surface."""
    _pack_ = 1
    _fields_ = [
        ("controlFlag", c_ubyte),  # Control flags for leveling behavior
        ("precision", c_float)      # Desired precision in mm
    ]

# ==================== External Motor Control ====================

class EMotor(Structure):
    """External motor control (e.g., conveyor belt, rail)."""
    _fields_ = [
        ("temp", c_float)
    ]
"""External motor control with distance/position target."""
    _pack_ = 1
    _fields_ = [
        ("index", c_byte),        # Motor index (which external motor)
        ("isEnabled", c_byte),    # Enable/disable motor
        ("speed", c_int32),       # Motor speed
        ("distance", c_uint32)    # Target distance to move
    ]

# ==================== Robot Arm Configuration ====================

ArmOrientation = enum(
    LeftyArmOrientation=0,   # Left-handed arm orientation (elbow left)
    RightyArmOrientation=1)  # Right-handed arm orientation (elbow right)

# ==================== JOG (Manual Jogging) Motion ====================
# Jog mode allows manual control of the robot in real-time

class JOGJointParams(Structure):
    """Velocity and acceleration parameters for joint-space jogging."""
...
clas"""Velocity and acceleration parameters for Cartesian-space jogging."""
    _pack_ = 1
    _fields_ = [
        ("xVelocity", c_float),        # X-axis velocity (mm/s)
        ("yVelocity", c_float),        # Y-axis velocity (mm/s)
        ("zVelocity", c_float),        # Z-axis velocity (mm/s)
        ("rVelocity", c_float),        # Rotation velocity (deg/s)
        ("xAcceleration", c_float),    # X-axis acceleration (mm/s²)
        ("yAcceleration", c_float),    # Y-axis acceleration (mm/s²)
        ("zAcceleration", c_float),    # Z-axis acceleration (mm/s²)
        ("rAcceleration", c_float)     # Rotation acceleration (deg/s²)
    ]

class JOGCommonParams(Structure):
    """Common velocity/acceleration ratio multipliers for all jog operations."""
class JOGCoordinateParams(Structure):
    _pack_ = 1
    _fields_ = [
        ("xVelocity", c_float),
        ("yVelocity", c_float),
    """Linear interpolation parameters for jogging."""
    _pack_ = 1
    _fields_ = [
        ("velocity",  c_float),      # Linear velocity
        ("acceleration",  c_float)   # Linear acceleration
    ]

# Jog command enumeration (button press states)
JC = enum(JogIdle=0,         # No jog command
          JogAPPressed=1,     # Axis A positive direction
          JogANPressed=2,     # Axis A negative direction
          JogBPPressed=3,     # Axis B positive direction
          JogBNPressed=4,     # Axis B negative direction
          JogCPPressed=5,     # Axis C positive direction
          JogCNPressed=6,     # Axis C negative direction
          JogDPPressed=7,     # Axis D positive direction
          JogDNPressed=8,     # Axis D negative direction
          JogEPPressed=9,     # Axis E positive direction
          JogENPressed=10)    # Axis E negative direction

class JOGCmd(Structure):
    """Jog command specifying joint/coordinate mode and direction."""  c_float)
    ]


JC = enum(JogIdle=0,
  ==================== PTP (Point-to-Point) Motion ====================
# PTP mode moves the robot from current position to a target position

class PTPJointParams(Structure):
    """Velocity and acceleration parameters for joint-space PTP motion."""
          JogBPPressed=3,
          JogBNPressed=4,
          JogCPPressed=5,
          JogCNPressed=6,
...
class JOGCmd(Structure):
    ...
    _fields_ = [
        ("isJoint", c_byte),
        ("cmd", c_byte)
    """Velocity and acceleration parameters for Cartesian-space PTP motion."""
    _fields_ = [
        ("xyzVelocity", c_float),      # Linear velocity for XYZ axes (mm/s)
        ("rVelocity", c_float),        # Rotational velocity (deg/s)
        ("xyzAcceleration", c_float),  # Linear acceleration (mm/s²)
        ("rAcceleration", c_float)     # Rotational acceleration (deg/s²)
    ]

class PTPLParams(Structure):
    """Linear interpolation parameters for PTP motion."""_float),
...
class PTPCoordinateParams(Structure):
    _fields_ = [
        ("xyzVelocity", c_float),
    """Jump motion parameters (lifts over obstacles during PTP)."""
    _pack_ = 1
    _fields_ = [
        ("jumpHeight", c_float),  # How high to lift during jump (mm)
        ("zLimit", c_float)       # Maximum Z height limit (mm)
    ]

class PTPCommonParams(Structure):
    """Common velocity/acceleration ratio multipliers for all PTP operations."""
    _fields_ = [      # Velocity scaling factor (0-100%)
        ("accelerationRatio", c_float)   # Acceleration scaling factor (0-100%)
    ]

# PTP motion modes
PTPMode = enum(
    PTPJUMPXYZMode=0,      # Jump motion in XYZ coordinates
    PTPMOVJXYZMode=1,      # Joint motion to XYZ target
    PTPMOVLXYZMode=2,      # Linear motion to XYZ target

    PTPJUMPANGLEMode=3,    # Jump motion in joint angles
    PTPMOVJANGLEMode=4,    # Joint motion to angle target
    PTPMOVLANGLEMode=5,    # Linear motion to angle target

    PTPMOVJANGLEINCMode=6, # Incremental joint angle motion
    PTPMOVLXYZINCMode=7,   # Incremental linear XYZ motion
    PTPMOVJXYZINCMode=8,   # Incremental joint XYZ motion

    PTPJUMPMOVLXYZMode=9)  # Jump then linear motion
...
InputPin = enum(InputPinNone=0,  # No input pin
                InputPin1=1,      # Digital input pin 1
                InputPin2=2,      # Digital input pin 2
                InputPin3=3,      # Digital input pin 3
                InputPin4=4,      # Digital input pin 4
                InputPin5=5,      # Digital input pin 5
                InputPin6=6,      # Digital input pin 6
                InputPin7=7,      # Digital input pin 7
                InputPin8=8)      # Digital input pin 8

InputLevel = enum(InputLevelBoth=0,  # Trigger on any level change
                  InputLevelLow=1,   # Trigger on low (0V)
                  InputLevelHigh=2)  # Trigger on high (5V/3.3V)

OutputPin = enum(
    SIGNALS_O1=1,  # Digital output pin 1
    SIGNALS_O2=2,  # Digital output pin 2
    SIGNALS_O3=3,  # Digital output pin 3
    SIGNALS_O4=4,  # Digital output pin 4
    SIGNALS_O5=5,  # Digital output pin 5
    SIGNALS_O6=6,  # Digital output pin 6
    SIGNALS_O7=7,  # Digital output pin 7
    SIGNALS_O8=8)  # Digital output pin 8

class PTPCmd(Structure):
    """Point-to-point motion command with target pose."""evelHigh=2)
...
OutputPin = enum(
    ...
    SIGNALS_O6=6,
    SIGNALS_O7=7,
    """Device usage statistics and counters."""
    _pack_ = 1
    _fields_ = [
        ("deviceRunTime",  c_uint64),  # Total runtime in milliseconds
        ("devicePowerOn",  c_uint32),  # Power-on count
        ("devicePowerOff", c_uint32)   # Power-off count
    ]

class PTPWithLCmd(Structure):
    """PTP command with additional linear parameter (e.g., conveyor sync)."""
        ("z", c_float),
        ("rHead", c_float)
    ]
...
# Continuous path mode for smooth trajectory following

class CPParams(Structure):
    """Continuous path planning parameters."""
    _pack_ = 1
    _fields_ = [
        ("planAcc", c_float),       # Planning acceleration
        ("juncitionVel", c_float),  # Junction/corner velocity
        ("acc", c_float),           # Execution acceleration
        ("realTimeTrack",  c_byte)  # Real-time tracking enable flag
    ]

ContinuousPathMode = enum(
    CPRelativeMode=0,  # Relative to current position
    CPAbsoluteMode=1)  # Absolute coordinates

class CPCmd(Structure):
    """Continuous path motion command."""tinuous path   ##################
...
class CP2Cmd(Structure):
  ==================== ARC (Circular Arc) Motion ====================
# Arc motion for smooth circular paths

class ARCPoint(Structure):
    """Point definition for arc motion (defines circle geometry)."""
        ("cpMode", c_byte),
        ("x", c_float),
        ("y", c_float),
        ("z", c_float),
...
class CPCommonParams(Structure):
    _pack_ = 1
    """Velocity and acceleration parameters for arc motion."""
    _pack_ = 1
    _fields_ = [
        ("xyzVelocity", c_float),      # Linear velocity along arc (mm/s)
        ("rVelocity", c_float),        # Rotational velocity (deg/s)
        ("xyzAcceleration", c_float),  # Linear acceleration (mm/s²)
        ("rAcceleration", c_float)     # Rotational acceleration (deg/s²)
    ]
...
class ARCCmd(Structure):
    """Arc motion command defined by intermediate and end points."""
    _pack_ = 1
    _fields_ = [
        ("cirPoint", ARCPoint),  # Intermediate point on arc
        ("toPoint", ARCPoint)    # End point of arc
    ]

class CircleCmd(Structure):
    """Complete circle motion command."""
    _pack_ = 1
    _fields_ = [
        ("cirPoint", ARCPoint),  # Point on circle circumference
        ("toPoint", ARCPoint)    # Ending point (completes circle)
    ]

class ARCCommonParams(Structure):
    """Common velocity/acceleration ratio multipliers for arc operations."""
...
class WAITParams(Structure):
    """Wait/delay parameters."""
    _pack_ = 1
    _fields_ = [
        ("unitType", c_byte)  # Time unit (seconds, milliseconds, etc.)
    ]

class WAITCmd(Structure):
    """Wait command to insert delays in command queue."""
    _pack_ = 1
    _fields_ = [
        ("waitTime", c_uint32)  # Wait duration (units depend on WAITParams)
    ]

# Trigger modes for conditional execution
TRIGMode = enum(
    TRIGInputIOMode=0,  # Trigger on digital I/O state
    TRIGADCMode=1)      # Trigger on analog ADC value

# Trigger conditions for digital I/O
TRIGInputIOCondition = enum(
    TRIGInputIOEqual=0,     # Trigger when equal to threshold
    TRIGInputIONotEqual=1)  # Trigger when not equal to threshold

# Trigger conditions for ADC (analog input)
TRIGADCCondition = enum(
    TRIGADCLT=0,  # Less than (<)
    TRIGADCLE=1,  # Less than or equal (<=)
    TRIGADCGE=2,  # Greater than or equal (>=)
    TRIGADCGT=3)  # Greater than (>)

class TRIGCmd(Structure):
    """
    Trigger command for conditional execution.
    Pauses command queue until trigger condition is met.
# ==================== GPIO and I/O Control ====================

GPIOType = enum(
    GPIOTypeDummy=0,  # Undefined/disabled GPIO
    GPIOTypeDO=1,     # Digital output
    GPIOTypePWM=2,    # PWM (Pulse Width Modulation) output
TRIGMode = enum(
    TRIGInputIOMode=0,
    TRIGADCMode=1)
...
def load():
    if platform.system() == "Windows":
        print("In order to run smoothly, the DLL use 64-bit system. Please, ensure that your python environment is also 64-bits.")
        print("The Python environment i: ", platform.architecture())
...
def SetDebugEnable(api, flag=False):
    ...
    return result  # Added by: Gustavo Eismann (2022-05-21)
