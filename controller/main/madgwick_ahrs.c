...
volatile float rgx = 0.0f, rgy = 0.0f, rgz = -1.0f;         // rotated acceleration vector of gravity
...
void MadgwickAHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz)
{
    ...
    // Rotate normal gravity vector
    RotateGravityAccelerationVector(0,0,-1);
}
...
void RotateGravityAccelerationVector(float gx, float gy, float gz)
{
     ...
     * 
     *  where:
     *      gv_ = gravity vector component (gvx for x component, gvy for y component and gvz for z component of the gravity vector);
     *      g_  = normal gravity vector components (0, 0, -1);
     * 
     * */

    float r11 = 1 - 2*q2*q2 - 2*q3*q3;
    float r12 = 2*q1*q2 - 2*q3*q0;
    float r13 = 2*q1*q3 + 2*q2*q0;
    float r21 = 2*q1*q2 + 2*q3*q0;
    float r22 = 1 - 2*q1*q1 - 2*q3*q3;
    float r23 = 2*q2*q3 - 2*q1*q0;
    float r31 = 2*q1*q3 - 2*q2*q0;
    float r32 = 2*q2*q3 + 2*q1*q0;
    float r33 = 1 - 2*q1*q1 - 2*q2*q2;

    rgx = r11 * gx + r21 * gy + r31 * gz;
    rgy = r12 * gx + r22 * gy + r32 * gz;
    rgz = r13 * gx + r23 * gy + r33 * gz;
}
